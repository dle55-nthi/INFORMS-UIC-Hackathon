...
function Chat() {
  ...
  const [authenticatedPatient, setAuthenticatedPatient] =
    useState<AuthenticatedPatient | null>(null);
  ...
  const isAuthenticated = Boolean(authenticatedPatient);
  ...
  const authenticate = useCallback(async () => {
    const fullName = credentialName.trim();
    if (!fullName) return;
    setCredentialError("");
    setAuthLoading(true);
    try {
      const result = (await agent.stub.authenticatePatient(fullName)) as {
        success?: boolean;
        message?: string;
      ...
      if (!result?.success || !result.patient) {
        ...
        setCredentialError(
          result?.message ?? "Could not verify a patient with this name."
        );
        return;
      }

      setAuthenticatedPatient(result.patient);
      setCredentialName(
        `${result.patient.first ?? ""} ${result.patient.last ?? ""}`.trim()
      );
        ...
        parts: [
          {
            type: "text",
            text:
              "Authenticated patient context for this session:\n" +
              `PATIENT ID: ${result.patient.patient}\n` +
              `Name: ${result.patient.first} ${result.patient.last}\n` +
              "Use this patient as the default context and call getPatientFullHistory before explaining costs."
          }
        ]
      });
    } catch {
      setCredentialError("Credential check failed. Please try again.");
    } finally {
      setAuthLoading(false);
    }
  }, [agent.stub, credentialName, sendMessage]);
        ...
        <div className="max-w-3xl mx-auto px-5 py-3">
          {!isAuthenticated ? (
            <div className="rounded-xl border border-kumo-line bg-kumo-elevated p-3">
              <Text size="sm" bold>
                Enter full name to unlock patient history
              </Text>
              <div className="mt-1">
                <Text size="xs" variant="secondary">
                  Demo credential: patient full name (for example, Giovanni
                  Paucek).
                </Text>
              </div>
              <div className="mt-3 flex gap-2">
                ...
                <Button
                  ...
                  disabled={!credentialName.trim() || authLoading}
                >
                  {authLoading ? "Checking..." : "Unlock"}
                </Button>
              </div>
              {credentialError && (
                <div className="mt-2 text-kumo-danger">
                  <Text size="xs">{credentialError}</Text>
                </div>
              )}
            </div>
          ) : (
            <div className="rounded-xl border border-kumo-line bg-kumo-elevated p-3 flex items-center justify-between gap-3">
              <div>
                <Text size="sm" bold>
                  Authenticated: {authenticatedPatient!.first}{" "}
                  {authenticatedPatient!.last}
                </Text>
                <Text size="xs" variant="secondary">
                  PATIENT ID: {authenticatedPatient!.patient} | Total
                  ED/Inpatient Cost: $
                  {Number(
                    authenticatedPatient!.ed_inpatient_total_cost ?? 0
                  ).toLocaleString()}
                </Text>
              </div>
              <Button
        ...
        >
          ...
          <div className="flex items-end gap-3 rounded-xl border border-kumo-line bg-kumo-base p-3 shadow-sm focus-within:ring-2 focus-within:ring-kumo-ring focus-within:border-transparent transition-shadow">
            ...
            <InputArea
              ...
              disabled={!connected || !isAuthenticated || isStreaming}
              rows={1}
